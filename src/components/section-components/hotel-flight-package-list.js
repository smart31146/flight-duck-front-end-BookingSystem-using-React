...
const HotelFlightPackageList = () => {
    ...
    const [originCodeToApi] = useGlobalState("originCodeToApi")
    ...
    const getCacheFlightHotelsPackage = () => {
        ...
        console.log({
            ...
            body: JSON.stringify({
                "originplace": originCodeToApi,
                "destinationplace": destinationCodeToApi,
    ...
    const sortSearchResultsBasedOnPrices = (rule) => {
        if(!loading  && hotelFlightPackageList.length > 3) {
            setTimeout(() => {
                let price = rule;
    ...
    const flight = () => {
        if(!loading  && hotelFlightPackageList.length > 3) {
            ...
            console.log("HEY LOADING IS ", loading)
            console.log("HEY hotelFlightPackageList IS ", hotelFlightPackageList)
